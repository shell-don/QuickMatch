...
@section('header', 'Créer une Clé API')

@section('content')
<form action="{{ route('admin.api-keys.store') }}" method="POST" class="max-w-2xl">
    @csrf
    ...
    <div class="card">
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="partner_name" id="partner_name" required class="input mt-1" value="{{ old('partner_name') }}">
                @error('partner_name')
            ...
            <div>
                ...
                <input type="email" name="partner_email" id="partner_email" class="input mt-1" value="{{ old('partner_email') }}">
                @error('partner_email')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="mt-2 space-y-2">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="permissions[]" value="users:read" class="rounded text-indigo-600" {{ in_array('users:read', old('permissions', ['users:read'])) ? 'checked' : '' }}>
                        <span class="ml-2 text-sm text-gray-700">users:read</span>
                    </label>
                    <br>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="permissions[]" value="stats:read" class="rounded text-indigo-600" {{ in_array('stats:read', old('permissions', [])) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <input type="date" name="expires_at" id="expires_at" class="input mt-1" value="{{ old('expires_at') }}">
                @error('expires_at')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="ip_whitelist" id="ip_whitelist" class="input mt-1" placeholder="192.168.1.1, 10.0.0.1" value="{{ old('ip_whitelist') }}">
                @error('ip_whitelist')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="inline-flex items-center">
                    <input type="checkbox" name="send_email" value="1" class="rounded text-indigo-600" {{ old('send_email') ? 'checked' : '' }}>
